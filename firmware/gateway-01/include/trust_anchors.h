/**
 * @file trust_anchors.h
 * @brief BearSSL trust anchor for ISRG Root X1 (Let's Encrypt root CA).
 *
 * Used by SSLClient (OPEnSLab-OSU/SSLClient) to verify the TLS certificate
 * of sapi.ilha3d.com. The server certificate chain is:
 *
 *   ISRG Root X1  (embedded here as trust anchor)
 *     └── Let's Encrypt R13  (sent by server during TLS handshake)
 *           └── sapi.ilha3d.com  (sent by server during TLS handshake)
 *
 * Only the root CA needs to be embedded because the intermediates are sent
 * by the server itself during the TLS handshake (standard practice).
 *
 * Generation method:
 *   The DN and RSA public key bytes were extracted from the system trust store
 *   at /etc/ssl/certs/ISRG_Root_X1.pem using openssl and Python:
 *
 *     openssl x509 -in ISRG_Root_X1.pem -outform DER > isrg.der
 *     # DN:  cert_der[160:241]  (81 bytes, ASN1 offset from openssl asn1parse)
 *     # N:   openssl rsa -pubin -noout -modulus (512 bytes, 4096-bit RSA)
 *     # E:   65537 = {0x01, 0x00, 0x01}
 *
 * Certificate details:
 *   Subject : CN=ISRG Root X1, O=Internet Security Research Group, C=US
 *   Issuer  : (self-signed)
 *   Valid   : 2015-06-04 to 2035-06-04
 *   Key     : RSA 4096 bits
 *
 * Confirmed against sapi.ilha3d.com on 2026-03-06:
 *   openssl s_client -connect sapi.ilha3d.com:443 → depth=2 CN=ISRG Root X1
 *
 * @author Alexandre Nuernberg
 * @date 2026-03-06
 *
 * SPDX-License-Identifier: AGPL-3.0-or-later
 * Copyright (C) 2024–2026 Alexandre Nuernberg <alexandreberg@gmail.com>
 *
 * This program is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * This program is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. See the
 * GNU Affero General Public License for more details.
 *
 * You should have received a copy of the GNU Affero General Public License
 * along with this program. If not, see <https://www.gnu.org/licenses/>.
 */

#pragma once

#include <SSLClient.h>

// ─── ISRG Root X1 Distinguished Name (DER-encoded, 81 bytes) ─────────────────

static const uint8_t TA_DN[81] = {
    0x30, 0x4f, 0x31, 0x0b, 0x30, 0x09, 0x06, 0x03, 0x55, 0x04, 0x06, 0x13,
    0x02, 0x55, 0x53, 0x31, 0x29, 0x30, 0x27, 0x06, 0x03, 0x55, 0x04, 0x0a,
    0x13, 0x20, 0x49, 0x6e, 0x74, 0x65, 0x72, 0x6e, 0x65, 0x74, 0x20, 0x53,
    0x65, 0x63, 0x75, 0x72, 0x69, 0x74, 0x79, 0x20, 0x52, 0x65, 0x73, 0x65,
    0x61, 0x72, 0x63, 0x68, 0x20, 0x47, 0x72, 0x6f, 0x75, 0x70, 0x31, 0x15,
    0x30, 0x13, 0x06, 0x03, 0x55, 0x04, 0x03, 0x13, 0x0c, 0x49, 0x53, 0x52,
    0x47, 0x20, 0x52, 0x6f, 0x6f, 0x74, 0x20, 0x58, 0x31
};

// ─── ISRG Root X1 RSA Public Key — Modulus N (4096-bit, 512 bytes) ───────────

static const uint8_t TA_RSA_N[512] = {
    
};

// ─── ISRG Root X1 RSA Public Key — Exponent E (3 bytes = 65537) ──────────────

static const uint8_t TA_RSA_E[3] = { 0x01, 0x00, 0x01 };

// ─── BearSSL Trust Anchor Array ───────────────────────────────────────────────

/// Trust anchor array passed to SSLClient constructor.
/// Contains 1 entry: ISRG Root X1 (CA flag set — it is a root certificate authority).
static const br_x509_trust_anchor TAs[1] = {
    {
        { (unsigned char *)TA_DN, sizeof(TA_DN) },
        BR_X509_TA_CA,
        {
            BR_KEYTYPE_RSA,
            { .rsa = {
                (unsigned char *)TA_RSA_N, sizeof(TA_RSA_N),
                (unsigned char *)TA_RSA_E, sizeof(TA_RSA_E),
            }}
        }
    }
};
