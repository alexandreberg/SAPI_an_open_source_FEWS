...
/**
 * @file alert_log.php
 * @brief Admin page: read-only alert notification history.
 *
 * Displays all alert_log entries with optional filters by station,
 * level, and date range.  Paginated at 50 rows per page.
 *
...
$pageTitle  = 'Log de Alertas';
$activePage = 'alert_log';
...
$countStmt = $pdo->prepare(
    "SELECT COUNT(*) FROM alert_log al $whereSql"
);
...
$rowsStmt     = $pdo->prepare(
    "SELECT al.id, al.sent_at, s.name AS station_name,
            al.variable, al.level, al.event_type,
            al.value_at_trigger,
            ac.name AS contact_name,
            al.channel, al.status
     FROM alert_log al
     LEFT JOIN stations      s  ON s.id  = al.id_station
     LEFT JOIN alert_contacts ac ON ac.id = al.id_contact
     $whereSql
      ...
      <main class="app-main">
        ...
        <div class="app-content">
          <div class="container-fluid">
            ...
            <div class="card mb-3">
              <div class="card-body">
                <form method="get" action="alert_log.php" class="row g-2 align-items-end">
                  ...
                  <tbody>
                    ...
                    <?php else: ?>
                      <?php foreach ($rows as $r): ?>
                        ...
                        $statusBadge = $r['status'] === 'sent' ? 'text-bg-success' : 'text-bg-danger';
                        ?>
                        <tr>
                          ...
                          <td><?= $r['value_at_trigger'] !== null ? $r['value_at_trigger'] : '—' ?></td>
