...
"""
@file config_production_m6.py
@brief Production configuration for the SAPI real-time M6 MLR prediction pipeline (Issue #120).

All runtime paths are under PRODUCTION_DIR.  The model is trained once by
train_production_model_m6.py and then loaded by sapi_predict_m6.py on every cron run.

Constants here mirror the M6 backtesting config (results_final_m6_mlr/config.py) so
the production model is identical to the validated one.

M4 LightGBM is isolated in a separate folder (LightGBM_Production) and this module
...
PRODUCTION_DIR = "/home/ilha3d/SAPI/MLR_Production"
"""Root directory for all M6 production artefacts (gitignored at runtime)."""

MODEL_PATH      = os.path.join(PRODUCTION_DIR, "models", "model6_mlr.pkl")
"""Serialised Darts LinearRegressionModel — written by train_production_model_m6.py."""

STATE_PATH      = os.path.join(PRODUCTION_DIR, "state", "debounce_state_m6.json")
"""JSON file persisting per-station debounce counters across cron runs."""

LOG_LAST_PRED   = os.path.join(PRODUCTION_DIR, "logs", "ultima_predicao_m6.log")
"""Human-readable summary of the most recent M6 prediction run (overwritten each run)."""
...
Same key as M4 — both pipelines authenticate with the same credential."""

TELEGRAM_CONFIG_PATH = os.path.join(PRODUCTION_DIR, "config", "telegram_config.json")
"""JSON file with bot_token and chat_ids for Telegram alerts (chmod 600, gitignored).
Copy from /home/ilha3d/SAPI/LightGBM_Production/config/telegram_config.json."""
...
# ---------------------------------------------------------------------------
# Pre-processing (identical to results_final_m6_mlr/config.py)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Model hyper-parameters (identical to results_final_m6_mlr/config.py)
# ---------------------------------------------------------------------------
...
MLR_BUFFER_STEPS  = max(abs(l) for l in MLR_LAGS)   # = 24 steps = 2 h
"""Minimum history steps required before first prediction."""

# ---------------------------------------------------------------------------
# Alert thresholds (cm) per station — same as M4 until field-tuned
# ---------------------------------------------------------------------------
