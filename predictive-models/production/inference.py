...
"""
@file inference.py
@brief Single-step inference for the SAPI M4 LightGBM production pipeline (Issue #108).
...
def _parse_api_rows(rows: list[dict], value_key: str) -> pd.Series:
    ...
    @param rows       List of dicts from api_client.fetch_data().
    @param value_key  Key to use as the value column (e.g. 'value').
...
def build_level_series(rows: list[dict]) -> Optional[pd.Series]:
    ...
    @return Clean 5-min Series, or None if there are fewer than LGBM_BUFFER_STEPS rows.
    """
    ...
    if rs.notna().sum() < cfg.LGBM_BUFFER_STEPS:
        logger.warning(
            "build_level_series: only %d non-NaN steps after resampling (need %d)",
            rs.notna().sum(), cfg.LGBM_BUFFER_STEPS,
        )
        return None
...
def _build_merge_series_from_rows(rows: list[dict], now_naive: pd.Timestamp) -> pd.Series | None:
    ...
    MERGE data is hourly; each hourly total is forward-filled across the 12
    five-minute slots that follow it (same logic as the original CSV path).
...
def build_precip_series(
    ...
    series is built in results_final_m4_lgbm/real_utils.py::load_precip_5min.
    MERGE hourly totals are forward-filled and divided by 12 (5-min distribution).

    When merge_fallback is True, the function tries sources in order:
      1. Live MERGE data from the Hostinger API (field=merge).
      2. Local merge_precip.csv (last-resort if Hostinger is unreachable).
      3. Zero-filled Series (absolute last resort).
    ...
    if merge_fallback:
        # ── 1. Live MERGE from Hostinger API ────────────────────────────────
        try:
        ...
        # ── 3. Absolute last resort: zero series ─────────────────────────────
    ...
    # Station-02 reads every 15 min, so its last row can lag up to 15 min behind
    # the level series, causing Darts to raise "past_covariates not long enough".
...
def compute_gate(precip_series: pd.Series) -> tuple[float, bool]:
    ...
    @return Tuple (rolling_sum_mm, gate_active) where gate_active is True
            when rolling_sum_mm >= GATE_MM.
    """
...
def _classify_alert(preds: dict[int, float], station_id: int) -> str:
    ...
    Checks all four horizons; returns the highest level where any horizon
    prediction equals or exceeds the corresponding threshold.
...
def run_inference(
    ...
    """Run one prediction step for a station.

    @param model             Trained Darts LightGBMModel loaded from pkl.
    @param level_rows        Raw level API rows (fetch_data result).
    @param precip_rows       Raw precip API rows (fetch_data result for station-02).
    @param station_id        Station identifier (for thresholds).
    ...
    @return Dict with prediction results, or None if inference is not possible.
            Keys: station_id, timestamp, h_pred_{30,60,90,120},
                  precip_rolling, precip_source, gate_active,
                  raw_alert_level.
    """
    # ── Level ─────────────────────────────────────────────────────────────────
    ...
    if ts_level is None or len(ts_level) < cfg.LGBM_BUFFER_STEPS:
    ...
    if ts_precip is None:
        # Zero-precip fallback: build a constant-zero series aligned to level.
    ...
    # Suppress if gate is not active.
