...
@brief Telegram notification module for SAPI predictive alert transitions (Issue #110).

Sends a chart image (fired) or plain text (cleared) to all configured Telegram
chat IDs when the M4 LightGBM pipeline fires or clears a predictive alert.
...
The chart (fired only) shows:
  - Solid blue line: last 2 h of actual level readings
  - Dashed orange line + markers: +30/+60/+90/+120 min prediction horizons
  - Horizontal threshold lines: atenção / alerta / inundação
  - Title with station, alert level, and UTC timestamp
...
import matplotlib
matplotlib.use("Agg")  # headless backend — no display required on RPi
...
def _load_config() -> dict | None:
    ...
    Uses a module-level cache so the file is read only once per process.

    @return Dict with 'bot_token' and 'chat_ids', or None if the file is
            missing or malformed (Telegram will be silently skipped).
    """
    global _tg_config
...
# Colour palette and PT-BR labels for each alert level
...
def build_alert_chart(
    ...
    """Render a prediction snapshot chart for a fired alert.

    Generates a 10 × 5-inch PNG showing the last 2 h of measured level
    alongside the four prediction horizons, plus horizontal threshold lines.
    The image is returned as PNG bytes in memory — nothing is written to disk.

    @param result      Enriched prediction result dict (keys: h_pred_30,
                       h_pred_60, h_pred_90, h_pred_120, alert_level,
                       precip_rolling, precip_source, consecutive_steps).
    @param level_rows  Raw API rows [{timestamp: str, value: float}, ...]
                       for the station's level (last LEVEL_FETCH_HOURS hours).
    @param station_id  Station identifier (used for per-station thresholds).
    @return PNG image as bytes.
    """
    ts_now = datetime.now(timezone.utc)

    # ── Parse raw level history ───────────────────────────────────────────────
    ...
    # Clip to last 2 h (matches the inference buffer window)
    cutoff = ts_now - timedelta(hours=2)
    ser = ser[ser.index >= cutoff]

    # ── Prediction points (+30, +60, +90, +120 min) ───────────────────────────
    ...
    if not ser.empty:
        ...
        # Thin bridge from last measured point to first prediction
        ax.plot(
    ...
    ax.plot(
        ...
        label="Previsão LightGBM (cm)", zorder=4,
    )

    # Horizon labels above each prediction dot
    ...
    # Threshold lines (dotted, coloured by level)
    ...
    ax.set_title(
        f"Alerta Preditivo — Estação {station_id} — {alert_label} — {ts_str}",
    ...
    # ── Encode to PNG bytes (no disk write) ────────────────────────────────────
    buf = io.BytesIO()
...
_TG_TIMEOUT = 30  # seconds per request
...
def _tg_url(bot_token: str, method: str) -> str:
    ...
    @param method     Telegram method name (e.g. 'sendPhoto').
...
def send_telegram_photo(
    ...
    @return True on success (Telegram ok=true), False on any error.
    """
...
def _build_fired_caption(result: dict, station_id: int) -> str:
    """Build the image caption for a fired alert notification.
    ...
    lines = [
        f"[SAPI] Alerta Preditivo — {alert_label}",
...
def _build_cleared_message(result: dict, station_id: int) -> str:
    """Build the text message for a cleared alert notification.
    ...
    lines = [
        f"[SAPI] Alerta Preditivo Normalizado — Estação {station_id}",
        f"Timestamp: {ts_str}",
        "",
...
def notify_alert(
    ...
    """Send Telegram notifications for a predictive alert transition.

    - fired  → generates a chart image and sends it via sendPhoto to each chat ID.
    - cleared → sends a plain text message via sendMessage to each chat ID.
    - No-op  if neither fired nor cleared, or if Telegram config is missing.

    Errors are logged but never raised so a Telegram failure cannot interrupt
    the main prediction pipeline.

    @param result      Enriched prediction result dict (h_pred_*, alert_level, etc.).
    @param level_rows  Raw level API rows — used for chart rendering on fired.
    @param station_id  Station identifier.
    @param fired       True when the alert state transitions from none to active.
    @param cleared     True when the alert state transitions from active to none.
    @return PNG chart bytes on fired (None if chart generation failed), None on cleared.
    """
