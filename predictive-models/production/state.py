...
"""
@file state.py
@brief Debounce state persistence for the SAPI production prediction pipeline (Issue #108).

Reads and writes a JSON file at STATE_PATH.  Each cron run calls update() for
every station, which increments or resets the consecutive-step counter and
determines whether an alert has fired or cleared since the last run.

State schema (per station, keyed by str(station_id)):
  {
    "consecutive_steps":   int,  // how many back-to-back steps the raw signal has been active
    "last_alert_level":    str,  // last debounced classification ('none'/'atencao'/'alerta'/'inundacao')
    ...
    "last_update":          str  // ISO-8601 UTC timestamp of last update
  }
"""
...
def update(
    ...
    the displayed/logged level every run (dashboard, predictions table) and
    is unrelated to the notification decision below.
    ...
    if gated_alert_level != "none":
        st["consecutive_steps"] = min(
            ...
            cfg.DEBOUNCE_STEPS * 2,   # cap to prevent runaway
        )
    else:
