...
"""
@file config.py
@brief Configuration for M4 LightGBM continuous backtesting — v3 (Simulado Contínuo).

Identical model hyper-parameters and thresholds as real_data_v2 so that results
are directly comparable.  Adds continuous-simulation-specific parameters:
WINDOW_STEPS and DEBOUNCE_STEPS.

SIM_START and SIM_END are resolved at runtime from ts_level.start_time() /
ts_level.end_time() so the simulation always covers the full available dataset
without requiring a hardcoded date update after each MySQL dump import.
"""
...
SYNTHETIC_DATA_DIR = ""
"""Placeholder — darts_utils.py references this at import time; never called here."""
...
"""Reprocessed from raw level_cm via reprocess_level_spike_return_filter.py
(Issue #211): the original station01_level.csv's level_delta_cm inherited a
production filter_level.php echo-oscillation capture bug that contaminated
Events 14 and 20. See that script's module docstring for the full pipeline
(production elastic filter -> existing 40cm filter -> new spike-and-return
look-ahead) and Documentation/predictive_model/auditoria_lead_time_rmse_20eventos.md
for the incident."""
...
"""Historical MERGE precipitation archive (2025-01-01 -> 2026-04-04). Was
merge_precip.csv (the rolling 7-day production cache) — that file cannot
reach the 2025 training events and today only has data from early Sep 2026,
so re-running training with it either crashed or trained on zero MERGE rows.
results_final_m6_mlr/config.py already used this fixed value since 2026-05-16;
M4 was never updated to match. See
Documentation/predictive_model/auditoria_lead_time_rmse_20eventos.md #2.4.1-2.4.2."""
STATION02_CSV = os.path.join(DATA_DIR, "station02_precip.csv")
EVENTS_CSV = os.path.join(_HERE, "event_windows_v3.csv")
"""Event catalogue versioned inside this folder."""

# ---------------------------------------------------------------------------
# Pre-processing (identical to v2)
# ---------------------------------------------------------------------------
...
that made the forecasts jump on every report."""
...
# ---------------------------------------------------------------------------
# Model horizons (identical to v2)
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Alert thresholds (cm) — identical to v2 (Issue #99)
# ---------------------------------------------------------------------------
...
has fallen to threshold - REARM_HYSTERESIS_CM. 5 cm reproduced the author's
visual reading of the multi-peak test events (E25, E31, E33, E34, E35); 8 and
10 cm are compared in Documentation/predictive_model/base_reescrita_dissertacao.md
before the value is fixed."""

PRED_PERSISTENCE_STEPS = 3
"""A forecast horizon's alarm fires only after its forecast has stayed at or
above the threshold for this many consecutive 5-min steps (15 min) — the same
persistence as the operational alarm (debounce 3). Lead time is measured to the
firing instant, so an isolated one-step forecast spike is never credited."""

CAUSALITY_MAX_LEAD_MIN = 180
"""Earliest pairing of a predicted alarm with an observed crossing, in minutes
before it — 1.5x the longest horizon (120 min). See lead_time_utils.py for the
full search window (it also stops at the previous re-arm and at this
occurrence's own re-arm)."""

# ---------------------------------------------------------------------------
# LightGBM hyper-parameters (identical to v2)
# ---------------------------------------------------------------------------
...
LGBM_BUFFER_STEPS = max(abs(lag) for lag in LGBM_LAGS)
...
# ---------------------------------------------------------------------------
# Continuous simulation parameters (new in v3)
# ---------------------------------------------------------------------------

WINDOW_STEPS = LGBM_BUFFER_STEPS * 2
"""Sliding history window size in steps (48 × 5 min = 240 min = 4 hours).

LightGBM only reads lags up to LGBM_BUFFER_STEPS deep so 2× provides a safe
margin without growing memory unbounded — mirroring how the production RPi
keeps only a recent buffer in RAM.
"""

DEBOUNCE_STEPS = [1, 2, 3]
"""Debounce scenarios tested for the operational alert filter.

1 step  (5 min)  = Scenario A: any single prediction above threshold fires alert.
2 steps (10 min) = Scenario B: two consecutive predictions required.
3 steps (15 min) = Scenario C: three consecutive predictions required.
"""

EVENT_MERGE_GAP_MIN = 120
"""Maximum gap in minutes between detected flood steps to merge into one period.

Detected flood periods separated by less than this are treated as a single
continuous event for plotting purposes.
"""
