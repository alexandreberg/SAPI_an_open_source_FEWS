...
"""
@file 04_lead_time_m4.py
@brief Lead time (advance warning) analysis for the M4 LightGBMModel.
...
The rule (occurrences with re-arm hysteresis, 15-min persistence, search window
bounded by the previous and own re-arm) lives in lead_time_utils.py (Issue
#229) and is shared with results_final_m6_mlr/04_lead_time_m6.py and
plot_events_audit_pdf.py, so M4 and M6 are measured identically.
...
Output (results_final_m4_lgbm/output/):
  lead_time_summary_m4.csv          — one row per event × threshold × occurrence
  lead_time_atencao_m4.png
  lead_time_alerta_m4.png
  lead_time_inundacao_m4.png

Usage (run from the results_final_m4_lgbm/ folder):

    cd AppTest/raspberry/predictive_model_darts/results_final_m4_lgbm/
    python3 04_lead_time_m4.py
"""
...
def _plot_lead_time_single(
    ...
    ax.set_title(
        f"M4 LightGBM — Tempo de antecipação  ≥ {threshold_cm:.0f} cm ({level_name})",
        fontsize=10,
    )
    ...
    path = os.path.join(out_dir, f"lead_time_{slug}_m4.png")
...
def main() -> None:
    """Compute and save M4 lead time analysis."""
    out_dir = config.OUTPUT_DIR
    os.makedirs(out_dir, exist_ok=True)

    print("=" * 66)
    print("  M4 LightGBM — Lead Time Analysis (Issue #138)")
    print("=" * 66)
    ...
    # ── 3. Save CSV ───────────────────────────────────────────────────────────
    csv_path = os.path.join(out_dir, "lead_time_summary_m4.csv")
    ...
    print("=== M4 LEAD TIME ANALYSIS DONE ===\n")
