...
"""
@file 02_precip_gate_m6.py
@brief Precipitation gate post-analysis for M6 MLR continuous backtesting.

Post-processes ``output/continuous_predictions_m6.csv`` (produced by
``01_model6_mlr_continuous.py``) by applying a rolling precipitation gate before
the debounce alert filter — exactly the same gate logic used for M4 in
results_final_m4_lgbm/02_precip_gate_analysis.py.

Gate logic (gate before debounce):

    raw_signal  = (ANY horizon prediction >= threshold_cm)
                  AND (rolling_precip_6h >= gate_mm)
    gated_alert = debounce(raw_signal, n_consecutive)

Sweeps GATE_MM_VALUES = [0, 5, 10, 15, 20] mm.  The gate_mm=0 row is a sanity
baseline that must reproduce alert_analysis_m6.csv exactly.

Usage (run from the results_final_m6_mlr/ folder):

    cd AppTest/raspberry/predictive_model_darts/results_final_m6_mlr/
    python3 02_precip_gate_m6.py

Output (results_final_m6_mlr/output/):

    precip_gate_analysis_m6.csv               — 45 rows (5 gate_mm × 3 thresholds × 3 debounce)
    precip_gate_timeline_atencao_gateNmm.png  — comparison plot for Atenção (50 cm)
    precip_gate_timeline_alerta_gateNmm.png   — comparison plot for Alerta (60 cm)
    precip_gate_timeline_inundacao_gateNmm.png — comparison plot for Inundação (75 cm)
"""
...
"""Gate thresholds for which timeline comparison plots are generated."""
...
def load_predictions(out_dir: str) -> pd.DataFrame:
    """Load the continuous predictions CSV produced by 01_model6_mlr_continuous.py.

    @param out_dir  Path to the output directory.
    @return DataFrame with columns: timestamp, h_obs, h_pred_{30,60,90,120}, ...
    @raises SystemExit if the file does not exist.
    """
    path = os.path.join(out_dir, "continuous_predictions_m6.csv")
    if not os.path.exists(path):
        print(
            f"ERROR: {path} not found.\n"
            "Run 01_model6_mlr_continuous.py first to generate continuous_predictions_m6.csv."
        )
        sys.exit(1)
...
def run_sanity_check(df_gate: pd.DataFrame, alert_csv_path: str) -> None:
    """Verify that gate_mm=0 rows reproduce alert_analysis_m6.csv exactly.

    @param df_gate        Full precip_gate_analysis_m6 DataFrame (all gate_mm values).
    @param alert_csv_path Path to alert_analysis_m6.csv from 01_model6_mlr_continuous.py.
    """
    if not os.path.exists(alert_csv_path):
        print(f"  [WARN] {os.path.basename(alert_csv_path)} not found — skipping sanity check.")
        return

    df_orig = pd.read_csv(alert_csv_path)
    df_base = df_gate[df_gate["gate_mm"] == 0].copy()

    print(f"\n  Sanity check (gate_mm=0 must match {os.path.basename(alert_csv_path)}):")
    all_ok = True
    ...
    if all_ok:
        print("  All gate_mm=0 rows match alert_analysis_m6.csv. ✓")
    else:
...
def _annotate_events(
    ...
    @param alert_ser        Boolean alert Series indexed by timestamp.
    @param threshold_cm     Used for label y-position.
    """
    for _, evt in events_at_thresh.iterrows():
        t_start = pd.Timestamp(evt["analysis_start"])
        t_end   = pd.Timestamp(evt["analysis_end"])
        window  = (
            ...
            if not alert_ser.empty else pd.Series(dtype=bool)
        )
...
def plot_gate_comparison_timeline(
    ...
    @param gate_mm_target  Gate threshold shown in the gated panel title.
    ...
    fp_red_pct = (
        f"  (redução {fp_orig - fp_gated} FP = {100*(fp_orig - fp_gated)/fp_orig:.0f}%)"
        if fp_orig > 0 else ""
    )
    ax2.set_title(
        f"Com gate (≥{gate_mm_target} mm / {gate_hours:.0f}h, debounce=3)  —  FP={fp_gated}{fp_red_pct}",
        fontsize=9,
    )
    ...
    fig.suptitle(
        f"Filtro de Precipitação ({MODEL_LABEL}) — {threshold_name} (≥ {threshold_cm:.0f} cm)  "
        f"|  Verde=VP  Laranja=FN  Vermelho=FP",
...
def main() -> None:
    """Run precipitation gate post-analysis on existing continuous_predictions_m6.csv."""
    out_dir = config.OUTPUT_DIR
    os.makedirs(out_dir, exist_ok=True)

    print("=" * 66)
    print(f"  {MODEL_LABEL} — Filtro de Precipitação")
    print("=" * 66)

    # ── 1. Load M6 predictions ───────────────────────────────────────────────
    print("\n[1/4] Loading continuous_predictions_m6.csv ...")
    df_preds = load_predictions(out_dir)

    # ── 2. Load precipitation and events ─────────────────────────────────────
    ...
    gate_csv = os.path.join(out_dir, "precip_gate_analysis_m6.csv")
    df_gate.to_csv(gate_csv, index=False)
    print(f"\n  Saved {gate_csv}  ({len(df_gate)} rows)")

    alert_csv = os.path.join(out_dir, "alert_analysis_m6.csv")
    ...
    print(f"=== {MODEL_LABEL} FILTRO DE PRECIPITAÇÃO DONE ===\n")
